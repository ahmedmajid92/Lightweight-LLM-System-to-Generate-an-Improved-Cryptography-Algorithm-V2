...
# ---------- Key schedule (research prototype) ----------
...
def ks_sha256_kdf(key: bytes, *, rounds: int, out_len: int, seed: int = 0) -> List[bytes]:
    ...
    This is *not* a standardized cipher key schedule. It's a simple, reproducible baseline for experiments.
    """
...
# ---------- Permutations (AES ShiftRows, column-major) ----------

# For AES-like state layout (column-major): index = col*4 + row
...
def builtin_components() -> Dict[str, Component]:
    ...
    comps["ks.sha256_kdf"] = Component(
        ...
        kind="KEY_SCHEDULE",
        description="Deterministic SHA-256 KDF key schedule (research baseline)",
